...
- Agent config: the pre-call model picker chooses Gemini 3.8 Live or Gemini 3.8 Live Extended Thinking; only Extended Thinking receives the selected low/medium/high thinking level. The route sends these public IDs directly to the preview gateway. NEXT_GOOGLE_API_KEY is required.
...
- Next.js serves the app and route handlers on the first available port starting at 3000.
...
- Require NEXT_GOOGLE_API_KEY for Gemini MLLM sessions.
