...
- `NEXT_GOOGLE_API_KEY`: Gemini API key (server only).

The greeting and agent UID are constants in demo code. The pre-call UI selects Gemini 3.8 Live or Gemini 3.8 Live Extended Thinking, with a low/medium/high slider for Extended Thinking.
...
- App + API routes use the first available port starting at 3000. Read the
  Local URL from startup output; two demos started together choose separate ports.
- `pnpm run build && pnpm start` serves the standalone production build and
  also selects an available port.
...
| --- | --- | --- | --- |
| Agent never joins | Invite route or credential mismatch | `pnpm run doctor` and invite route logs | Verify the three secrets and invite payload |
