...
```json
{ "requester_id": "1234", "channel_name": "ai-conversation-...", "model": "models/gemini-3.8-live-extended-thinking", "thinking_level": "high" }
```
...
`model` is `models/gemini-3.8-live` or `models/gemini-3.8-live-extended-thinking`;
only Extended Thinking accepts `thinking_level` (`low`, `medium`, `high`, default
`medium`). Gemini 3.8 Live sends no thinking parameter. The route sends public
IDs directly to the preview gateway. Validation failures return `400`;
server failures return `500`.
...
- `NEXT_GOOGLE_API_KEY` (required for Gemini MLLM)

The greeting and agent UID are constants in the demo. The model and available
thinking level are selected in the pre-call UI.
