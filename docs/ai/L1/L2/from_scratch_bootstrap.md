...
2. Add `env.local.example` with `NEXT_PUBLIC_AGORA_APP_ID`, `NEXT_AGORA_APP_CERTIFICATE`, `NEXT_GOOGLE_API_KEY`.
...
4. Implement `POST /api/invite-agent` with `AgoraClient`, `Agent`, a single `GeminiLive` voice provider, RTM enabled, metrics enabled, and `{ requester_id, channel_name }` input.
...
- `DEFAULT_AGENT_UID` stays aligned between the browser and invite route.
