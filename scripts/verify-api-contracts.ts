...
process.env.NEXT_GOOGLE_API_KEY = 'fake-google-key';
...
async function verifyInviteAgentValidation() {
  ...
  for (const invalid of [
    { model: 'unknown' },
    { model: 'walkie-talkie' },
    { model: 'models/gemini-3.8-live', thinking_level: 'high' },
    { model: 'models/gemini-3.8-live-extended-thinking', thinking_level: 'extreme' },
  ]) {
    const response = await inviteAgent(new NextRequest('http://localhost:3000/api/invite-agent', {
      body: JSON.stringify({ requester_id: 'user-1', channel_name: 'test-channel', ...invalid }),
      method: 'POST',
    }));
    assert(response.status === 400, 'POST /api/invite-agent should reject invalid model or thinking level');
  }
}
...
async function verifyInviteAgentSuccess() {
  ...
  Agent.prototype.createSession = (function (this: Agent, sessionConfig: unknown) {
    capturedModel = this.mllm?.params?.model;
    capturedThinkingLevel = this.mllm?.params?.thinking_level;
  ...
  try {
    for (const selection of [
      { model: 'models/gemini-3.8-live', expectedVendorModel: 'models/gemini-3.8-live', expectedThinking: undefined },
      { model: 'models/gemini-3.8-live-extended-thinking', expectedVendorModel: 'models/gemini-3.8-live-extended-thinking', thinking_level: 'low', expectedThinking: 'low' },
      { model: 'models/gemini-3.8-live-extended-thinking', expectedVendorModel: 'models/gemini-3.8-live-extended-thinking', thinking_level: 'medium', expectedThinking: 'medium' },
      { model: 'models/gemini-3.8-live-extended-thinking', expectedVendorModel: 'models/gemini-3.8-live-extended-thinking', thinking_level: 'high', expectedThinking: 'high' },
    ]) {
      const request = new NextRequest('http://localhost:3000/api/invite-agent', {
        body: JSON.stringify({ requester_id: 'user-4321', channel_name: 'test-channel', model: selection.model, ...(selection.thinking_level ? { thinking_level: selection.thinking_level } : {}) }),
        method: 'POST',
      });
      const response = await inviteAgent(request);
      const body = await getJson(response);
    ...
    assert(capturedModel === selection.expectedVendorModel,
      'POST /api/invite-agent should map the public 3.8 ID to the current preview vendor model');
    assert(capturedThinkingLevel === selection.expectedThinking,
      'POST /api/invite-agent should use the selected thinking level only when available');
    ...
    assert(
      ...
      'POST /api/invite-agent should use the hardcoded agent UID for the session',
    );
    assert(
...
async function verifyStopConversationSuccess() {
  ...
  globalThis.fetch = async (input, init) => {
    stopUrl = String(input);
    const headers = new Headers(init?.headers);
    featureHeader = headers.get('agora-feature') ?? '';
  ...
  try {
    ...
    assert(
      stopUrl.includes('/preview/api/conversational-ai-agent/v2/projects/') &&
        stopUrl.endsWith('/agents/mock-agent-id/leave'),
      'POST /api/stop-conversation should use the preview endpoint for the requested agent',
    );
    assert(
      featureHeader === 'gemini-live',
      'POST /api/stop-conversation should send the Gemini preview feature header',
    );
    assert(
      authorization.startsWith('agora token='),
      'POST /api/stop-conversation should authenticate with Token007',
    );
  } finally {
